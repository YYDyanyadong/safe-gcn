...
from MyApplication.safe_semi_supervise_gcn.dataset_op import edgeIndexOnCut
...
'''
# 只获取前140个样本之间的关联无向图
data.edge_index = edgeIndexOnCut(data.edge_index, 139)

# 把样本集和标签都拆解成train,val和test三个部分
...
class Net(torch.nn.Module):   # 这个网络的精髓就在这里,具体的卷积过程在GCNConv里面
    def __init__(self): # 可以尝试在初始化上放入样本和标签的参数，这样在导出模型的时候就可以使用参数输入了。
        ...
        # 加一层隐含层
        # self.conv1 = GCNConv(data.num_features, 80, cached=True, normalize=not args.use_gdc)
        # self.conv2 = GCNConv(80, 16, cached=True, normalize=not args.use_gdc)
    ...
    def forward(self):   # 这里的forward除了self是没有参数的，因为数据集data已经在下一行传入了。
        ...
        x = x[0:140, :]
        # 设置我自己的参数
        # edge_index = edgeIndexOnCut(data.edge_index, 139)
        ...
        # 原文章处理方式，两层隐含层
        x = F.relu(self.conv1(x, edge_index, edge_weight))   # self.conv1(x, edge_index, edge_weight)又是以对象的形式调用方法，会触发__call__()，而该函数会调用GCNConv中的forward()函数
...
# 对多个数据集计算准确率
def acc_set(set_list, label_list, model):
    accs = []
    for set, label in zip(set_list, label_list):
...
@torch.no_grad()
def test( ):    # 在测试部分是不计算梯度的，所以如果需要改动模型并利用模型中已经学习好的参数那么就在test()里面改。
...
def gcnRun():
    ...
    torch.save(test(), 'supervisor_test.plk')
